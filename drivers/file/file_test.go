...
func init() {
	driver, err := file.NewDriver("../../.test/.expiring-cache")
	if err != nil {
		panic(err)
	}

	expiringStash = cache.Open(driver, time.Second * 2, time.Second * 2)
}


var testPutCases = map[string]string{
...
func TestFile_Put(t *testing.T) {
	for name, contents := range testPutCases {
		assert.NoError(t, expiringStash.Put(name, strings.NewReader(contents)))
	}

	assert.Error(t, expiringStash.Put("no-reader", ""))
